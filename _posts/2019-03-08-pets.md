...
```
    ...
    87306240it [00:00, 98083066.62it/s]
...
Total time: 06:49 <p><table border="1" class="dataframe">
  <thead>
...
```
    ...
    PosixPath('/root/.fastai/data/oxford-iiit-pet/images/models/pets-stage-1.pth')
...
For example, if we look at the top left image, the model predicts that the breed to be beagle but it turned out to be american pit bull terrier, with a loss of 7.26, and a 0% prediction probability of actual class. We will talk about loss in the future, for now just know the algorithm predict that the probability of the image showing an american pit bull terrier (actual class) to be 0%.
...
Seems like resnet34 tend to misclassify 'egyptian mau' as 'bengal', any dog expert here?
...
Total time: 07:47 <p><table border="1" class="dataframe">
  <thead>
    <tr style="text-align: left;">
      <th>epoch</th>
      <th>train_loss</th>
...
That's right, the model has a validation error rate of 5.07%, which is equivalent to a 94.93% of accuracy after learning rate tuning. **Can you believe we best the top researchers from University of Oxford in 2012 with just a few lines of codes? **
